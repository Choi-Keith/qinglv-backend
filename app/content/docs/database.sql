...
CREATE TABLE `post_content` (
    ...
    `a_link` varchar(128) NOT NULL DEFAULT '' comment '链接分享'
    `creator_id` bigint(20) unsigned NOT NULL COMMENT '发布者',
    `created_at` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
    `updated_at` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP  COMMENT '修改时间',
    `deleted_at` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '删除时间',
...
CREATE TABLE `post_feed`(
    ...
    `version` bigint NOT NULL DEFAULT '1' COMMENT '版本号',
    PRIMARY KEY (`id`),
    UNIQUE KEY (`idx_post_feed`) (`user_id`),
    UNIQUE KEY (`idx_post_feed`) (`post_id`),
...
CREATE TABLE `article_feed`(
    ...
    `version` bigint NOT NULL DEFAULT '1' COMMENT '版本号',
    PRIMARY KEY (`id`),
    UNIQUE KEY (`idx_article_feed`) (`user_id`),
    UNIQUE KEY (`idx_article_feed`) (`article_id`),
